...
You can verify any **Three Card Poker** bet in three ways. All of them run **the same math the server used** —
nothing is trusted, everything is recomputed.

## What you need

From your Three Card Poker bet history, each bet shows:
...
[`paytable.json`](https://github.com/maczo-co/originals-threecard/blob/main/paytable.json); the verifier
...
```
https://verify.maczo.co/originals/?game=threecard&server=<revealed>&commit=<hash>&client=<you>&nonce=<n>&params=<url-encoded JSON>
```

## Method 2 — Browser, manual

1. Open https://verify.maczo.co/originals/?game=threecard (works fully offline; view source).
...
**Your bet inputs (params).** This game takes: `play` — pass them as the `params` JSON, keys exactly as your bet history shows them.

## Method 3 — Node.js CLI

```bash
git clone https://github.com/maczo-co/originals-threecard
cd originals-threecard
